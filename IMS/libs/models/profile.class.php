...
class profile extends database {
    ...
    public function update_academic_details() {
        //echo $_POST['initial_acad_counter'];
        //echo $_POST['acad_counter'];
        /* $academic_details = get_academic_details();
        for($id=1;$id<$_POST['acad_counter'];$id++) {
            foreach($academic_details as $academics) {
                if($academics['course']==$_POST['iniatialcourse'.$id])
            }
        }*/
        for($rid=1;$rid<$_POST['initial_acad_counter'];$rid++) {
            //echo $_POST['removedid'.$id];
            ...
            $query = "UPDATE academics SET course = :course, math_score = :math_score, percentage = :percentage, year_of_passout = :year_of_passout
