
<?php ob_start() ?>
<?php $count = 0;?>
<!-- TABLE: LATEST ORDERS -->
<!-- Content Header (Page header) -->
<section class="content-header">
  <h1>
    Candidate list
  </h1>
  <ol class="breadcrumb">
    <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
    <li class="active"><a href="#">candiadtes</a></li>
  </ol>
</section>
<div class="row">
  <div class="col-xs-12">
    <div class="box box-default collapsed-box">
      <div class="box-header with-border">
        <h3 class="box-title">Filter</h3>
        <div class="box-tools pull-right">
          <button type="button" class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-plus"></i>
          </button>
        </div>
        <!-- /.box-tools -->
      </div>
  <!-- /.box-header -->
      <div class="box-body">
        <table>
          <tbody>
            <tr>
              <td>Minimum percent:</td>
              <td><input type="text" id="min" name="min"></td>
              <td>Maximum percent:</td>
              <td><input type="text" id="max" name="max"></td>
            </tr>
            <tr>
              <td>min Math mark:</td>
              <td><input type="text" id="min_math" name="min_math"></td>
              <td>Max math mark:</td>
              <td><input type="text" id="max_math" name="max_math"></td>
            </tr>
          </tbody>
        </table>
      </div>
    <!-- /.box-body -->
    </div>
  <!-- /.box -->
  </div>
</div>

<!-- Main content -->
<section class="content">
  <div class="row">
    <div class="col-xs-12">
      <div class="box">
        <div class="box-header">
          <h3 class="box-title">Full list</h3>
        </div>
        <!-- /.box-header -->
        <div class="box-body">
          <table id="candidateList" class="table table-bordered table-striped">
            <thead>
              <th><h3>Reg.No</h3></th>
              <th><h3>Name</h3></th>
              <th><h3>Email</h3></th>
              <th><h3>Phone</h3></th>
              <th><h3>Course</h3></th>
              <th><h3>Percentage</h3></th>
              <th><h3>Maths_score</h3></th>
              <th><h3>Experience</h3></th>
            </thead>
            <tbody>
              <?php foreach($candidates as $candidate):?>
                <tr>
                  <td><?=$candidate['id']?></td>
                  <td><a href="/admin/score_management/showprofile?pid=<?= $candidate['id']?>"><?= $candidate['name']?></a></td>
                  <td><?= $candidate['email']?></td>
                  <td><?=$candidate['phone']?></td>
                  <?php foreach($acadamics as $acadamic):?>
                    <?php if($candidate['id']==$acadamic['part_id']):?>
                      <td><?= $acadamic['course']?></td>
                      <td><?=$acadamic['percentage']?></td>
                      <td><?= $acadamic['math_score']?></td>
                    <?php endif; ?>
                  <?php endforeach;?>
                  <?php foreach($experience as $exper):?>
                    <?php if($candidate['id']==$exper['part_id']):?>
                      <td><?=$exper['position']?></td>
                    <?php endif; ?>
                  <?php endforeach;?>
                </tr>
              <?php endforeach;?>
            </tbody>
            <tfoot>
              <th><h3>Reg.No</h3></th>
              <th><h3>Name</h3></th>
              <th><h3>Email</h3></th>
              <th><h3>Phone</h3></th>
              <th><h3>Course</h3></th>
              <th><h3>Percentage</h3></th>
              <th><h3>Maths_score</h3></th>
              <th><h3>Experience</h3></th>
            </tfoot>
          </table>
        </div>
        <!-- /.box-body -->
      </div>
      <!-- /.box -->
    </div>
    <!-- /.col -->
  </div>
  <!-- /.row -->
</section>
<!-- /.content -->
<!-- /.box -->
<?php $content=ob_get_clean();?>
<?php include 'layout.tpl.php'?>